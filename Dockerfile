...
# 4.downloading deb packages on Ubuntu2204
FROM ubuntu:22.04 as deb_jammy
...
RUN apt update && \
    apt install -d -y \
      ...
      socat && \
    cd /var/cache/apt/archives && \
    tar zcvf /tmp/basic_jammy.tar.gz *.deb
...
COPY --from=deb_jammy /tmp/*.tar.gz /packages/
